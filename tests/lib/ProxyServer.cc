...
public:
    ...
    void worker_thread()
    {
...

      {
        ...
        if ( _auth ) {
          confTemplate +=
              "auth_param basic program /usr/libexec/squid/basic_ncsa_auth %3%/htaccess\n"
        ...
        if ( canContinue ) canContinue = TestTools::writeFile( varConfFile, str::Format(confTemplate) % _port % confPath % (Pathname(TESTS_SRC_DIR)/"data"/"proxyconf") );
      }
